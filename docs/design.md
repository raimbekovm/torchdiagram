...
Subscripting a tensor is where holding that equality takes the most work, because the two tracers disagree a level above naming: fx records the Python operator, export records the ATen op it became. `x[:, 0]` is `operator.getitem` to one and `aten.select.int` to the other, and `x[..., :-1]` is `aten.slice.Tensor`. Two rules close the gap. Every form of subscript is labeled `index` on both sides, since the level a diagram reads at is "this indexes the tensor", not which overload did it. And where export lowers one subscript per axis — `x[:, 0, :-1]` becomes a `select` then a `slice`, against fx's single node — consecutive indexing ops that came from the same source line and feed nothing but each other fold back into one box. The source line is what separates that from two subscripts written on separate lines, which fx draws as two nodes as well. It cannot separate two subscripts *chained on one line* — `x[0][1]` lowers to the same two `select` ops as `x[:, 0]` does, on the same line, and the export graph carries no column information to tell them apart. Folding gets the common case right at the cost of that one, which is the better trade: a multi-axis subscript is ordinary and a chained one on a single line is not.
