...
The mirror of that is a tensor built from nothing. `torch.arange(8)` hands fx no proxy to trace through, so fx runs the call while tracing and keeps the result as a graph attribute: the diagram reads `_tensor_constant0`, a name torch invented, where export reads `arange`, and whatever the model computes from that tensor is folded away with it and never drawn at all. Nothing links the attribute back to the call, so there is no recovering it after the fact. The twelve factories a model plausibly writes in `forward()` are held back for the duration of a trace instead, each recording a node rather than running — but only when the model's own code is what called it, since torch builds tensors constantly on its way down to an operation and handing one of those a proxy breaks a trace over something the model never wrote. A model can also want the tensor rather than the operation, iterating a range or reading an element out of it to branch on, and no proxy stands in for that; there is no telling which model that is in advance, so a trace that fails with the factories held back is retried without them and draws exactly what it drew before. Not folding constants also makes what the model does to them visible, which is how dtype casts came to be labeled `to` on both sides — fx keeps `x.float()` as a method of its own where export lowers every cast the same way.

A literal — `torch.tensor([1.0, 2.0])` — takes the same treatment on the fx side and a different one on export's, which keeps no call for it at all: the literal is lifted out of `forward()` into a constant of the exported program, and what stands in the graph is `lift_fresh_copy` reading the constant back, followed by `detach_` to strip the gradient history a fresh tensor would otherwise carry. The lifted constant is not the model's attribute and is left out of the diagram, which is what asking the _original_ module for the attribute name settles — a registered buffer answers and `lifted_tensor_0` does not. `lift_fresh_copy` becomes the box, labeled `tensor` after what the reader wrote, and the `detach_` on top of it folds in. Only the in-place overload folds: `x.detach()` lowers to `detach`, which is an operation a reader wrote and fx draws, so a model writing `torch.tensor([...]).detach()` keeps both boxes on both frontends. `torch.as_tensor` draws the same `tensor` box, since `lift_fresh_copy` is all export records and it cannot tell the two spellings apart; `torch.Tensor` is a class rather than a function and cannot be held back at all without breaking the `isinstance` checks torch runs while tracing.
