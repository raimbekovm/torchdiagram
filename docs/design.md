...
fx does not record a source line by default, and turning on `Tracer.record_stack_traces` to get one captures and _formats_ a whole traceback per node. Tracing a GPT-2 goes from 18 ms to 511 ms — the formatting reads every source file it names, which is warm on a second trace and cold on the only one a CLI run performs. `_LineTracer` reads the innermost frame outside torch instead, which is the only part ever consulted and costs nothing measurable (19 ms). fx never descends into a `torch.nn` module, since they are all leaves, so that frame is always the `forward()` being traced.
