...
The same `nn_module_stack` walk yields `scope`/`scope_class`, so `aggregate_blocks()` works on export graphs unchanged. On the models both frontends can trace, they emit identical graphs, which is asserted in the test suite. The one place they still differ is the name of a purely functional op that ATen lowers under another name: indexing a tensor (`qkv[0]`) is a `getitem` node under fx and a `select` node under export. Layer boxes, structure, scopes, and shapes agree.
