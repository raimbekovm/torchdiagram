...
def _preamble(theme: Theme) -> list[str]:
    ...
    corner = _num(theme.corner_radius)
    line_width = _num(theme.stroke_width)
    return [
...
def to_tikz(graph: Graph, *, theme: Theme = DEFAULT) -> str:
    ...
    for node in graph.nodes:
        ...
        lines.append(rf"\node[{style}{placement}] ({node.id}) {{{_node_text(node, theme)}}};")
        previous = node.id
...
def _node_text(node: Node, theme: Theme) -> str:
